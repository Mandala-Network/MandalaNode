...
const envVars: EnvVarDefinition[] = [
    {
        key: 'MANDALA_NODE_PORT',
        description: 'The TCP port on which the Mandala Node server will listen.',
        default: '7777',
        validate: (val) => !isNaN(Number(val)) && Number(val) > 0 && Number(val) < 65536,
    },
    {
        key: 'MANDALA_NODE_SERVER_BASEURL',
        description: 'The base URL of the Mandala Node server.',
        default: 'http://localhost:7777',
        validate: (val) => val.startsWith('http')
    },
    {
        key: 'MYSQL_DATABASE',
        description: 'The name of the MySQL database used by the Mandala node.',
        default: 'mandala_db',
...
(async () => {
    console.log(chalk.cyanBright('Welcome to the Mandala Node Setup Wizard!'));
    ...
    console.log(`You can now run your Mandala node with these settings.\n`);
})();